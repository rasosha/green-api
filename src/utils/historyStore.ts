        ...
        if (history) {
          ...
          if (chatInstance.history) {
            if (chatInstance.history.find((message) => message.idMessage === history[0].idMessage)) {
              newHistory = chatInstance.history.map((message) =>
                message.idMessage === history[0].idMessage ? history[0] : message,
              );
            } else {
  ...
  removeChat: ({ chatId }: ChatInstance) => {
    ...
    try {
      localStorage.setItem(
        ...
        JSON.stringify(
          newArr.map((item) => {
            return { chatId: item.chatId, name: item.name, avatar: item.avatar };
          }),
        ),
      );
    } catch (err) {
